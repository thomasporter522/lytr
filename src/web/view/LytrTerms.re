open Virtual_dom.Vdom;
open Tylr_core;
open LytrGrammar;
open LytrParser;
open LytrAbstractor;

let string_of_secondary_token = (s: secondary_token) =>
  switch (s) {
  | Whitespace(s) => s
  | Unlexed(s) => s
  };

let string_of_atom = (a: atom) =>
  switch (a) {
  | Numlit(n) => n
  | Identifier(s) => s
  };

let string_of_binop = binop =>
  switch (binop) {
  | Plus => "+"
  | Minus => "-"
  | Times => "*"
  | Divide => "/"
  | DoubleDivide => "//"
  | Modulo => "%"
  };

let string_of_primary_token = (t: primary_token) =>
  switch (t) {
  | BOF => "#"
  | EOF => "#"
  | TOP => "("
  | TCP => ")"
  | TOSB => "["
  | TCSB => "]"
  | TAtom(a) => string_of_atom(a)
  | TPlus => "+"
  | TMinus => "-"
  | TTimes => "*"
  | TDivide => "/"
  | TDoubleDivide => "//"
  | TModulo => "%"
  | TFactorial => "!"
  | TFun => "fun"
  | TArrow => "→"
  | TLet => "let"
  | TEquals => "="
  | TIn => "in"
  | TType => "type"
  | TCase => "case"
  | TPipe => "|"
  | TDoubleArrow => "⇒"
  | TEnd => "end"
  | TIf => "if"
  | TThen => "then"
  | TElse => "else"
  | TColon => ":"
  | TComma
  | TCommaTuple
  | TCommaList => ","
  };

let string_of_token = (t: token) =>
  switch (t) {
  | Primary(t) => string_of_primary_token(t)
  | Secondary(s) => string_of_secondary_token(s)
  };

let is_whitespace = (t: sharded(term)): bool =>
  switch (t) {
  | Unform(Secondary(Whitespace(_))) => true
  | _ => false
  };

let is_form = (t: sharded(term)): bool =>
  switch (t) {
  | Form(_) => true
  | _ => false
  };

let list_of_listr = (l: listr('a)): list('a) => {
  let rec helper = (l, acc) =>
    switch (l) {
    | Nil => acc
    | Cons(l, a) => helper(l, [a, ...acc])
    };
  helper(l, []);
};

/* Create styled view nodes that mimic the rich token system */
let mk_styled_token = (~text, ~classes, ()) => {
  Node.span(
    ~attrs=[Attr.classes(["token", "lytr-token", ...classes])],
    [Node.text(text)],
  );
};

let mk_operator_token = (~text, ()) =>
  mk_styled_token(~text, ~classes=["operator", "tile"], ());

let mk_paren_token = (~text, ()) =>
  mk_styled_token(~text, ~classes=["paren", "tile"], ());

let mk_atom_token = (~text, ()) =>
  mk_styled_token(~text, ~classes=["atom", "tile"], ());

let mk_unlexed_token = (~text, ()) =>
  mk_styled_token(~text, ~classes=["atom", "tile", "unlexed"], ());

let mk_half_space = mk_atom_token(~text=" ", ());

let mk_hole_circ = () => {
  Node.span([mk_styled_token(~text="○", ~classes=["hole"], ())]);
};

let mk_hole = {
  mk_hole_circ();
};

let mk_error_token = (~text, ()) =>
  mk_styled_token(~text, ~classes=["error", "grout"], ());

let mk_grout_dot = () => {
  Node.span([mk_styled_token(~text="·", ~classes=["lytr-grout"], ())]);
};

let mk_grout = (~font as _, ()) => {
  mk_grout_dot();
};

// this could be better. replace a space, centered, etc.
let rec put_hole_in_whitespace = (~font, terms: list(sharded(term))) => {
  let view = x => view_lytr_sharded(~font, x, failwith("todo"));
  let view_all = List.map(view);
  switch (terms) {
  | [] => [mk_hole]
  // | [Secondary(Whitespace(" "))] => [mk_hole]
  // | [Secondary(Whitespace(" ")), Secondary(Whitespace(" "))] => [
  //     mk_half_space,
  //     mk_hole,
  //     mk_half_space,
  //   ]
  | [h, ...t] => [view(h), mk_hole, ...view_all(t)]
  // | _ => [mk_hole] @ nodes
  };
}

/* Convert LytrParser terms to styled nodes WITH grout interspersion */
and view_lytr_terms = (~font, terms: terms, cursor: int): list(Node.t) => {
  let terms = list_of_listr(terms);
  if (List.for_all(is_whitespace, terms)) {
    put_hole_in_whitespace(~font, terms);
  } else {
    let rec render_with_grout = (terms, preceeded_by_form) => {
      switch (terms) {
      | [] => []
      | [term, ...terms] =>
        (preceeded_by_form && is_form(term) ? [mk_grout(~font, ())] : [])
        @ [view_lytr_sharded(~font, term, cursor)]
        @ render_with_grout(
            terms,
            preceeded_by_form && is_whitespace(term) || is_form(term),
          )
      };
    };
    render_with_grout(terms, false);
  };
}

and view_lytr_sharded = (~font, sharded: sharded(term), cursor: int): Node.t =>
  switch (sharded) {
  | Unform(Secondary(Unlexed(text))) => mk_error_token(~text, ())
  | Unform(Secondary(token)) =>
    let text = string_of_secondary_token(token);
    mk_atom_token(~text, ());
  | Unform(Shard(token)) =>
    let text = string_of_primary_token(token);
    mk_error_token(~text, ());
  | Form(term) => view_lytr_term(~font, term, cursor)
  }

and view_comma_terms = (~font, terms, cursor: int) => {
  let tuple_elements =
    List.fold_right(
      (terms, acc) =>
        [
          Node.span(~attrs=[], view_lytr_terms(~font, terms, cursor)),
          ...acc,
        ],
      terms,
      [],
    );
  List.fold_right(
    (elem, acc) =>
      switch (acc) {
      | [] => [elem]
      | _ => [elem, mk_operator_token(~text=",", ()), ...acc]
      },
    tuple_elements,
    [],
  );
}

and view_lytr_term_inner = (~font, term: term, cursor): Node.t => {
  switch (term) {
  | Tuple(inner_term_list) =>
    Node.span(
      ~attrs=[Attr.class_("lytr-tuple")],
      [mk_paren_token(~text="(", ())]
      @ view_comma_terms(~font, inner_term_list, cursor)
      @ [mk_paren_token(~text=")", ())],
    )
  | List(inner_term_list) =>
    Node.span(
      ~attrs=[Attr.class_("lytr-tuple")],
      [mk_paren_token(~text="[", ())]
      @ view_comma_terms(~font, inner_term_list, cursor)
      @ [mk_paren_token(~text="]", ())],
    )
  | InfixBinop(left, binop, right) =>
    let op_text = string_of_binop(binop);
    Node.span(
      ~attrs=[Attr.class_("lytr-binop")],
      view_lytr_left_child(~font, left, cursor)
      @ [mk_operator_token(~text=op_text, ())]
      @ view_lytr_right_child(~font, right, cursor),
    );
  | AssocBinop(left, binop, entries, right) =>
    let op_text = string_of_binop(binop);
    let entry_nodes =
      List.fold_right(
        (entry, acc) =>
          switch (entry) {
          | Empty(unforms) =>
            // acc
            view_lytr_child_unforms(~font, unforms)
            @ [mk_operator_token(~text=op_text, ())]
            @ acc
          | Term(u1, term, u2) =>
            view_lytr_child_unforms(~font, u1)
            @ [view_lytr_term(~font, term, cursor)]
            @ view_lytr_child_unforms(~font, u2)
            @ [mk_operator_token(~text=op_text, ())]
            @ acc
          },
        entries,
        [],
      );
    Node.span(
      ~attrs=[Attr.class_("lytr-assoc-binop")],
      view_lytr_left_child(~font, left, cursor)
      @ [mk_operator_token(~text=op_text, ())]
      @ entry_nodes
      @ view_lytr_right_child(~font, right, cursor),
    );
  | PrefixUnop(unop, child) =>
    let op_text =
      switch (unop) {
      | Minus => "-"
      | _ => failwith("impossible")
      };
    Node.span(
      ~attrs=[Attr.class_("lytr-unop")],
      [mk_operator_token(~text=op_text, ())]
      @ view_lytr_right_child(~font, child, cursor),
    );
  | PostfixUnop(child, unop) =>
    let op_text =
      switch (unop) {
      | Factorial => "!"
      | _ => failwith("impossible")
      };
    Node.span(
      ~attrs=[Attr.class_("lytr-unop")],
      view_lytr_left_child(~font, child, cursor)
      @ [mk_operator_token(~text=op_text, ())],
    );
  | Atom(atom) =>
    let text = string_of_atom(atom);
    mk_atom_token(~text, ());
  | Fun(params, body) =>
    Node.span(
      ~attrs=[Attr.class_("lytr-fun")],
      [mk_atom_token(~text="fun", ())]
      @ view_lytr_terms(~font, params, cursor)
      @ [mk_atom_token(~text="→", ())]
      @ view_lytr_right_child(~font, body, cursor),
    )
  | Ap(func, args) =>
    let arg_elements =
      List.fold_right(
        (terms, acc) =>
          [
            Node.span(~attrs=[], view_lytr_terms(~font, terms, cursor)),
            ...acc,
          ],
        args,
        [],
      );
    let interspersed_args =
      List.fold_right(
        (elem, acc) =>
          switch (acc) {
          | [] => [elem]
          | _ => [elem, mk_operator_token(~text=",", ()), ...acc]
          },
        arg_elements,
        [],
      );
    Node.span(
      ~attrs=[Attr.class_("lytr-ap")],
      view_lytr_left_child(~font, func, cursor)
      @ [mk_paren_token(~text="(", ())]
      @ interspersed_args
      @ [mk_paren_token(~text=")", ())],
    );
  | Let(bindings, body_terms, body) =>
    Node.span(
      ~attrs=[Attr.class_("lytr-let")],
      [mk_atom_token(~text="let", ())]
      @ view_lytr_terms(~font, bindings, cursor)
      @ [mk_atom_token(~text="=", ())]
      @ view_lytr_terms(~font, body_terms, cursor)
      @ [mk_atom_token(~text="in", ())]
      @ view_lytr_right_child(~font, body, cursor),
    )
  | Type(type_params, type_body, body) =>
    Node.span(
      ~attrs=[Attr.class_("lytr-type")],
      [mk_atom_token(~text="type", ())]
      @ view_lytr_terms(~font, type_params, cursor)
      @ [mk_operator_token(~text="=", ())]
      @ view_lytr_terms(~font, type_body, cursor)
      @ [mk_atom_token(~text="in", ())]
      @ view_lytr_right_child(~font, body, cursor),
    )
  | Case(scrutinee, branches) =>
    let branch_nodes =
      List.map(
        ((pattern, body)) =>
          Node.span(
            ~attrs=[Attr.class_("lytr-case-branch")],
            [mk_atom_token(~text="|", ())]
            @ view_lytr_terms(~font, pattern, cursor)
            @ [mk_atom_token(~text="⇒", ())]
            @ view_lytr_terms(~font, body, cursor),
          ),
        branches,
      );
    Node.span(
      ~attrs=[Attr.class_("lytr-case")],
      [mk_atom_token(~text="case", ())]
      @ view_lytr_terms(~font, scrutinee, cursor)
      @ branch_nodes
      @ [mk_atom_token(~text="end", ())],
    );
  | If(terms1, terms2, child) =>
    Node.span(
      ~attrs=[Attr.class_("lytr-if")],
      [mk_atom_token(~text="if", ())]
      @ view_lytr_terms(~font, terms1, cursor)
      @ [mk_atom_token(~text="then", ())]
      @ view_lytr_terms(~font, terms2, cursor)
      @ [mk_atom_token(~text="else", ())]
      @ view_lytr_right_child(~font, child, cursor),
    )
  | Asc(c1, c2) =>
    Node.span(
      ~attrs=[Attr.class_("lytr-asc")],
      view_lytr_left_child(~font, c1, cursor)
      @ [mk_atom_token(~text=":", ())]
      @ view_lytr_right_child(~font, c2, cursor),
    )
  | Arrow(c1, c2) =>
    Node.span(
      ~attrs=[Attr.class_("lytr-asc")],
      view_lytr_left_child(~font, c1, cursor)
      @ [mk_atom_token(~text="→", ())]
      @ view_lytr_right_child(~font, c2, cursor),
    )
  | DEBUG => mk_error_token(~text="DEBUG", ())
  };
}

and view_lytr_term = (~font, term: term, cursor: int): Node.t => {
  let node = view_lytr_term_inner(~font, term, cursor);
  /* Add outline to every term */
  Node.span(
    ~attrs=[Attr.classes(["lytr-term", "lytr-term-outlined"])],
    [node],
  );
}

and view_lytr_left_child = (~font, child: left_child, cursor): list(Node.t) =>
  switch (child) {
  | Hole => [mk_hole]
  | Term(term, se) =>
    [view_lytr_term(~font, term, cursor)]
    @ view_lytr_child_unforms(~font, se)
  }

and view_lytr_right_child = (~font, child: right_child, cursor): list(Node.t) =>
  switch (child) {
  | Hole => [mk_hole]
  | Term(se, term) =>
    view_lytr_child_unforms(~font, se)
    @ [view_lytr_term(~font, term, cursor)]
  }

and view_lytr_child_unforms = (~font, se: unforms): list(Node.t) => {
  switch (se) {
  | Nil => []
  | Cons(rest, unform) =>
    view_lytr_child_unforms(~font, rest)
    @ [
      switch (unform) {
      | Secondary(Whitespace(s)) => mk_atom_token(~text=s, ())
      | Secondary(Unlexed(s)) => mk_unlexed_token(~text=s, ())
      | Shard(token) =>
        let text = string_of_primary_token(token);
        mk_error_token(~text, ());
      },
    ]
  };
};

/* Rich view function using styled tokens */
let view_lytr_text = (~font, terms: terms, cursor: int): Node.t => {
  let styled_line = view_lytr_terms(~font, terms, cursor);
  Node.div(
    ~attrs=[Attr.classes(["block", "lytr-block"])],
    [Node.span(~attrs=[Attr.class_("line")], styled_line)],
  );
};
